...
<Note>
  Matching the WASM version in React: the `@rive-app/react-canvas` version is not the same as the underlying `@rive-app/canvas` version it wraps. The `rive.wasm` must match the canvas version. To find it, open your installed React runtime's <a href="https://github.com/rive-app/rive-react/blob/v{{versionReact}}/package.json#L38">package.json on GitHub</a>
   and read the `@rive-app/canvas` dependency — e.g. `rive-react@v4.33.0` pins `@rive-app/canvas@2.41.0`. Swap the tag in that URL to match the version you have installed.
</Note>
...
<link
  ...
  as="fetch"
  crossorigin
  href="https://unpkg.com/@rive-app/canvas@4.33.0/rive.wasm"
/>
...
// RuntimeLoader.setWasmUrl("https://cdn.example.com/assets/rive-2.41.0.wasm");
